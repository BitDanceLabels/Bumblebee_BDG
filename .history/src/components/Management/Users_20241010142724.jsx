...
const Users = () => {
  ...
  useEffect(() => {
    fetch(
      `http://127.0.0.1:8000/api/users/?page=${currentPage}&pageSize=${PAGE_SIZE}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          Authorization:
            'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJuaHV0cGhhbSIsImV4cCI6MTczMDk1MTA5NX0.TjliACbckGzJf2A0_L6ItR7w_DtMBA8NaIYaPNtpG-o' // Replace with your actual token
        }
      }
    )
      .then((response) => response.json())
      .then((data) => {
        // Update users and total user count
        ...
        setTotalUsers(data.totalUsers) // Assuming the API returns total number of users
      })
      ...
      <TableWrapper>
        ...
        <Pagination>
          <span className="hint-text">
            Showing <b>{users.length}</b> out of <b>{totalUsers}</b> entries
          </span>
