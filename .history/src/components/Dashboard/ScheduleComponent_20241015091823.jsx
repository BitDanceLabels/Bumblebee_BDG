            ...
            {filteredTeachers.map((teacher) => (
              <tr key={teacher.name}>
                ...
                {timeslots.map((slot, index) => {
                  // Check if there's any info for this teacher's schedule
                  const info = teacher.schedule[slot]
                  if (info) {
                    return (
                      <td key={index}>
                        <ClassInfo
                          ...
                          left={`${index * 100}%`} // Adjust left based on index
                        >
                          <div>{info.code}</div>
                    ...
                    // Render a td with border if there's no class info
                    return (
                      <td key={index} style={{ border: '1px solid #ccc' }}></td>
                    )
                  }
                })}
              </tr>
