...
const ScheduleComponent = () => {
  ...
  const calculateSessionPosition = (startTime, duration, totalWidth) => {
    ...
    // Tính vị trí phần trăm của thời gian bắt đầu
    const startPercent = (startInMinutes / dayTotalMinutes) * 100

    // Tính chiều rộng (phần trăm) dựa trên duration (thời gian kéo dài)
    const durationPercent = (duration / dayTotalMinutes) * 100

    // Tính vị trí tính theo pixel
    const positionPx = (startPercent / 100) * totalWidth
                ...
                {timeslots.map((time, index) => {
                  ...
                  if (classInfo && tableContainerRef.current) {
                    const { positionPx, widthPx } = calculateSessionPosition(
                      time,
                      classInfo.duration,
                      tableContainerRef.current.scrollWidth // Chỉ thực hiện nếu ref đã được gán
                    )
                    return (
