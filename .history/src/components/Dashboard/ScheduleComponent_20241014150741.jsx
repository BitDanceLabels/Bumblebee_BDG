...
const ScheduleComponent = () => {
  ...
  const calculateSessionPosition = (startTime, duration, totalWidth) => {
    // Tìm timeslot gần nhất trước thời gian bắt đầu
    const closestTimeSlotIndex =
      timeslots.findIndex((slot) => slot >= startTime) - 1

    const [startHour, startMinute] = startTime.split(':').map(Number)
    const [slotHour, slotMinute] = timeslots[closestTimeSlotIndex]
      .split(':')
      .map(Number)

    // Tính số phút từ timeslot bắt đầu
    const minutesDifference =
      startHour * 60 + startMinute - (slotHour * 60 + slotMinute)
    const percentFromSlot = (minutesDifference / 30) * 100 // Mỗi slot là 30 phút

    // Tính phần trăm của bảng (totalWidth)
    const positionPx =
      (closestTimeSlotIndex / timeslots.length) * totalWidth +
      (percentFromSlot / 100) * (totalWidth / timeslots.length)

    // Tính chiều rộng dựa trên thời lượng
    const durationPercent = (duration / (30 * timeslots.length)) * 100
    const widthPx = (durationPercent / 100) * totalWidth

    return { positionPx, widthPx }
  }
            ...
            {filteredTeachers.map((teacher) => (
              <tr key={teacher.name}>
                ...
                {timeslots.map((time, index) => {
                  const classInfo = teacher.schedule[time]
                  if (classInfo && tableContainerRef.current) {
                    const { positionPx, widthPx } = calculateSessionPosition(
                      time,
                      classInfo.duration,
                      tableContainerRef.current.scrollWidth
                    )
                    return (
                      <TableCell key={time} style={{ position: 'relative' }}>
                        <ClassInfo
                          style={{
                            position: 'absolute',
                            left: `${positionPx}px`,
                            width: `${widthPx}px`,
                            backgroundColor: getClassColor(classInfo.type)
                          }}
                        >
