          ...
          <tbody>
            {filteredTeachers.map((teacher) => (
              <React.Fragment key={teacher.name}>
                <tr>
                  <StickyTableCell
                    rowSpan={Object.keys(teacher.schedule).length}
                  >
                    {teacher.name}
                  </StickyTableCell>
                  {Object.entries(teacher.schedule).map(
                    ([time, info], index) => (
                      <React.Fragment key={time}>
                        {index === 0 ? null : <tr />}
                        <StickyTableCell>{time}</StickyTableCell>
                        <StickyTableCell>{info.code}</StickyTableCell>
                        <StickyTableCell>{info.teacher}</StickyTableCell>
                        <StickyTableCell>{info.duration} phút</StickyTableCell>
                        <StickyTableCell>{info.type}</StickyTableCell>
                      </React.Fragment>
                    )
                  )}
                </tr>
