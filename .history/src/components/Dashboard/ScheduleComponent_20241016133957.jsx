...
const ScheduleComponent = () => {
  ...
  useEffect(() => {
    const calculateTotalSessions = () => {
      ...
      db_teachers.forEach((teacher) => {
        count += teacher.schedule.length // Updated to reflect array format
      })
  ...
  const filteredTeachers = db_teachers.filter((teacher) => {
    return (
      ...
      teacher.schedule.some((classInfo) => {
        return (
          classInfo.teacher
            .toLowerCase()
          ...
          classInfo.time.includes(searchTime)
        )
      })
    )
  })
  ...
  const getFilteredSuggestions = (term, key) => {
    ...
    db_teachers.forEach((teacher) => {
      teacher.schedule.forEach((classInfo) => {
        if (classInfo[key].toLowerCase().includes(term.toLowerCase())) {
