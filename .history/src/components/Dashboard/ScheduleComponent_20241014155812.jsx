            ...
            {filteredTeachers.map((teacher) => (
              <tr key={teacher.name}>
                ...
                {timeslots.map((_, index) => {
                  const classInfo = teacher.schedule[timeslots[index]]
                  const colSpan = classInfo
                    ? calculateColSpan(classInfo.duration)
                    : 1
                  const colSpanFraction = classInfo
                    ? colSpan / Math.ceil(colSpan)
                    : 1

                  return (
                    <TableCell key={index} colSpan={Math.ceil(colSpan)}>
                      {classInfo && (
                        <ClassInfo
                          ...
                          style={{
                            position: 'absolute',
                            left: `${index * 50}px`, // Hoặc bạn có thể tính toán vị trí dựa trên index hoặc thời gian
                            width: `${(classInfo.duration / 540) * 100}%`,
