...
const DetailClassInfo = ({ isOpen, onClose, classInfo }) => {
  ...
  // Helper function to compare time in "HH:mm" format
  const formatTime = (date) =>
    date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })

  // Check if any changes have been made
  useEffect(() => {
    const initialStartTime = new Date(`1970-01-01T${classInfo.time}:00`)

    const isChanged =
      editedTeacher !== classInfo.teacher ||
      editedStudent !== classInfo.studentName ||
      editedStartTime.getTime() !== initialStartTime.getTime() || // Compare time using getTime()
