...
const CreateTeacherModal = ({
  ...
  const apiUrl =
    import.meta.env.VITE_URL_CREAT_TEACHER ||
  ...
  const handleSubmit = async () => {
    try {
      setIsLoading(true) // Hiển thị loading khi submit
      ...
      if (response.ok) {
        ...
        onSubmit() // Gọi hàm onSubmit để cập nhật lại dữ liệu bên ListOfTeacher
        onClose() // Đóng modal
        setIsLoading(false) // Tắt loading sau khi thành công
      } else {
        const errorData = await response.json()
        setIsLoading(false) // Tắt loading khi gặp lỗi
        alert(
    ...
    <>
      ...
      >
        ...
        <ButtonWrapper>
          <ButtonCancel
            type="button"
            onClick={onClose} // Chỉ đóng modal mà không hiển thị loading
          ...
          <ButtonSubmit
            type="button"
            onClick={handleSubmit} // Hiển thị loading và submit
            style={{ marginTop: '10px' }}
          >
            Submit
          </ButtonSubmit>
