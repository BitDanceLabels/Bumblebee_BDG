...
const SelectClass = styled.select`
  ...
  margin-bottom: 20px;
  font-size: 16px;
  height: 150px; /* Tăng chiều cao để dễ chọn nhiều mục */
  overflow-y: auto;
`
...
const TeacherTimeTable = () => {
  ...
  const [selectedClasses, setSelectedClasses] = useState([]) // State để lưu các buổi học được chọn
  ...
  // Xử lý chọn nhiều buổi học
  const handleSelectClasses = (event) => {
    ...
    setSelectedClasses(selectedOptions) // Cập nhật state với các buổi học được chọn
  }
      ...
      <FullModalOverlay show={isFullModalOpen}>
        <FullModal>
          <ModalContent>
            ...
            <SelectClass multiple={true} onChange={handleSelectClasses}>
              {/* Duyệt qua từng ngày và sự kiện */}
