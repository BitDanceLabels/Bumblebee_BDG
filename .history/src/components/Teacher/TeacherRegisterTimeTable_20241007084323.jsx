...
function addMinutesToTime(time, minutesToAdd) {
  ...
  const date = new Date() // Tạo một đối tượng Date mới
  date.setHours(hours)
  ...
  const newHours = String(date.getHours()).padStart(2, '0') // Định dạng lại giờ
  const newMinutes = String(date.getMinutes()).padStart(2, '0') // Định dạng lại phút

  return `${newHours}:${newMinutes}` // Trả về thời gian đã cộng thêm phút
}
...
const TeacherRegisterTimeTable = () => {
  ...
  const handleCellClick = (day, time) => {
    ...
    if (isAlreadySelected) {
      ...
      setModalMessage(
        `Bạn có muốn hủy lớp vào ${time} đến ${addMinutesToTime(
          time,
          30
        )} thứ ${day}?`
      )
      ...
      setModalMessage(
        `Bạn có muốn đăng ký lớp vào ${time} đến ${addMinutesToTime(
          time,
          30
        )} thứ ${day}?`
      )
  ...
  const handleMouseUp = () => {
    if (startCell && endCell) {
      ...
      if (anySelected) {
        setPendingAction('cancel')
        setModalMessage(
          `Bạn có muốn hủy khung giờ từ ${
            startCell.time
          } đến ${addMinutesToTime(endCell.time, 30)} thứ ${startCell.day}?`
        )
      } else {
        setPendingAction('register')
        setModalMessage(
          `Bạn có muốn đăng ký khung giờ từ ${
            startCell.time
          } đến ${addMinutesToTime(endCell.time, 30)} thứ ${startCell.day}?`
        )
      }
    ...
    // Sau khi hoàn thành thao tác, đặt lại trạng thái chọn
  ...
  const handleMouseOver = (day, time) => {
    if (isSelecting) {
      // Nếu day hiện tại khác với day ban đầu
      if (startCell && startCell.day !== day) {
        setIsSelecting(false) // Dừng thao tác chọn
        setModalMessage('Bạn không thể chọn các ô khác hàng.')
        setIsModalOpen(true) // Hiển thị modal cảnh báo
        return
      }
      setEndCell({ day, time }) // Cập nhật endCell nếu cùng hàng
    }
  }
  ...
  const handleSubmit = () => {
    ...
    // Đặt lại trạng thái sau khi thực hiện
    setStartCell(null)
    setEndCell(null)
    setIsSelecting(false)
  }
  ...
  const handleCloseModal = () => {
    ...
    // Đặt lại startCell, endCell và isSelecting
    setStartCell(null)
    setEndCell(null)
    setIsSelecting(false)
  }
