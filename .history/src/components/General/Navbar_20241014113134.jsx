...
const Navbar = ({ isHovered }) => {
  ...
  const renderParentItem = (parentItem, label, Icon) => {
    ...
    // Lấy phần tử con đầu tiên từ rolePermissions dựa trên role và parentItem
    const firstChildItem = rolePermissions[role]?.[parentItem]?.[0]

    if (!firstChildItem) return null // Nếu không có phần tử con thì không render
    ...
    return (
      ...
      >
        {/* Thay vì sử dụng parentItem, sử dụng firstChildItem */}
