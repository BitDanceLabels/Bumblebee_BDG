...
const Navbar = ({ isHovered }) => {
  ...
  const isParentActive = (parentItem) => {
    if (
      parentItem === 'classes' &&
      ['announcement', 'classes', 'listOfTeacher'].includes(activeItem)
    )
      return true
    if (
      parentItem === 'teacher' &&
      ['teacher-register', 'teacher-timetable', 'teacher-profile'].includes(
        activeItem
      )
    )
      return true
      ...
      <nav className="navbar">
        <ul>
          ...
          <ul className={`sub-menu ${isHovered ? 'show' : 'hide'}`}>
            ...
            {renderChildItem('classes', 'listOfTeacher', 'List Of Teacher')}
          </ul>
          ...
          <ul className={`sub-menu ${isHovered ? 'show' : 'hide'}`}>
            ...
            {renderChildItem('booking', 'courses', 'Courses')}
          </ul>
