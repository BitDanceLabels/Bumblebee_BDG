...
import CreateStudentModal from '../modal/CreateStudentModal' // Nhúng CreateStudentModal
import DeactiveStudentModal from '../modal/DeactiveStudentModal' // Nhúng DeactiveStudentModal
...
const ListOfStudent = () => {
  ...
  const [actionType, setActionType] = useState('') // Lưu trữ loại hành động: "Activate" hoặc "Deactive"
  ...
  const handleActionClick = (student, type) => {
    ...
    setActionType(type) // Đặt loại hành động (Activate hoặc Deactive)
    setConfirmationModalOpen(true) // Mở modal xác nhận
  }
  ...
  return (
    <div>
              ...
              <tr key={index}>
                ...
                <td>
                  {student.status === 'Active' ? (
                    <Button
                      onClick={() => handleActionClick(student, 'Deactive')}
                    >
                      Deactive
                    </Button>
                  ) : (
                    <Button
                      onClick={() => handleActionClick(student, 'Activate')}
                    >
                      Activate
                    </Button>
                  )}
      ...
      >
        ...
        <p>
          Are you sure you want to {actionType.toLowerCase()} this student:{' '}
          {selectedStudent?.name}?
        </p>
        <Button onClick={handleActionSubmit}>Yes</Button>
